...
class SemanticDrone(Dataset):
    ...
    def __getitem__(self, idx):
        with open(self.imgs[idx], "rb") as f:
            image = Image.open(f)
            image = np.array(image.crop((200,200,3000,3000)))
        image = torch.from_numpy(image).permute(2, 0, 1).float()
        label = None
        if self.msks:
            with open(self.msks[idx], "rb") as f:
                label = Image.open(f)
                label = np.array(label.crop((200,200,3000,3000)))
            label = rgb_to_labels(label, colors=[(i, c.color) for i,c in enumerate(all23)])
