...
class MT(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.loss = nn.CrossEntropyLoss(
            ignore_index=self.ignore_index, weight=torch.Tensor(self.weights)
        )
    ...
    def update_teacher(self):
        ...
        for param_t, param in zip(
            self.network2.parameters(), self.network1.parameters()
        ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.alpha > 0.0:
            ...
            cutmixed_inputs, cutmixed_probs = self.cutmix(
                input_batch=unsup_inputs, target_batch=teacher_probs
            )  # BxCxHxW
            ...
            loss_no_reduce = self.unsup_loss(cutmixed_logits, cutmix_preds)  # BxHxW
