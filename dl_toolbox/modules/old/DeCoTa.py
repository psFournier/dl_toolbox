...
class PseudoLabelling(BaseModel):
    def __init__(self, ema, supervised_warmup, *args, **kwargs):
    ...
    def update_teacher(self):
        ...
        for param_t, param in zip(
            self.teacher_network.parameters(), self.student_network.parameters()
        ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.current_epoch >= self.supervised_warmup:
            ...
            if self.pseudo_labelling:
                ...
                teacher_probs, teacher_preds = torch.max(
                    teacher_outputs.softmax(dim=1), dim=1
                )
                loss_no_reduce = self.unsup_loss(student_outputs, teacher_preds)
                teacher_certain = teacher_probs > 0.5
                pseudo_loss = torch.sum(teacher_certain * loss_no_reduce) / torch.sum(
                    teacher_certain
                )
