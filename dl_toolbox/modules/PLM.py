...
class PLM(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.loss1 = nn.BCEWithLogitsLoss(
            reduction="none", pos_weight=torch.Tensor(self.weights).reshape(1, -1, 1, 1)
        )
        ...
        self.unsup_loss = nn.BCEWithLogitsLoss(reduction="none")
    ...
    def training_step(self, batch, batch_idx):
        ...
        mask = torch.ones_like(
            onehot_labels, dtype=onehot_labels.dtype, device=onehot_labels.device
        )  # B,C,H,W
        ...
        if self.trainer.current_epoch > self.alpha_milestones[0]:
            ...
            mixup_inputs, mixup_targets = self.mixup2(
                unsup_inputs, onehot_pseudo_labels, inputs, onehot_labels
            )  # B,C,H,W
            mixup_outputs = self.network(mixup_inputs)  # B,C,H,W

            loss_no_reduce = self.unsup_loss(mixup_outputs, mixup_targets)  # B,C,H,W
