    ...
    with rasterio.open(path) as image_file:
        with rasterio.open(raster_path) as raster_file:
            left, bottom, right, top = rasterio.windows.bounds(
                window, transform=image_file.transform
            )
            rw = rasterio.windows.from_bounds(
                left, bottom, right, top, transform=raster_file.transform
            )
            image = raster_file.read(window=rw, out_dtype=np.float32)
...
def minmax(image, m, M):
    if isinstance(image, np.ndarray):
        return np.clip(
            (image - np.reshape(m, (-1, 1, 1))) / np.reshape(M - m, (-1, 1, 1)), 0, 1
        )
    elif isinstance(image, torch.Tensor):
        return torch.clip(
            (image - torch.reshape(m, (-1, 1, 1))) / torch.reshape(M - m, (-1, 1, 1)),
            0,
            1,
        )
    else:
