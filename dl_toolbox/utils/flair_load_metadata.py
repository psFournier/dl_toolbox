...
def load_data(path_data, path_metadata, val_percent=0.8, use_metadata=True):
    ...
    trainval_domains = [
        Path(path_trainval, domain) for domain in os.listdir(path_trainval)
    ]
    ...
    dict_train = _gather_data(
        train_domains, path_metadata, use_metadata=use_metadata, test_set=False
    )
    dict_val = _gather_data(
        val_domains, path_metadata, use_metadata=use_metadata, test_set=False
    )
    ...
    dict_test = _gather_data(
        test_domains, path_metadata, use_metadata=use_metadata, test_set=True
    )
...
def subset_debug(train, val, test, subset_size, use_metadata=True):
    ...
    if use_metadata == True:
        dict_train = {
            "IMG": [
                train["IMG"][u]
                for u in random.sample(range(0, len(train["IMG"])), subset_train)
            ],
            "MSK": [
                train["MSK"][u]
                for u in random.sample(range(0, len(train["IMG"])), subset_train)
        ...
        dict_val = {
            "IMG": [
                val["IMG"][u]
                for u in random.sample(range(0, len(val["IMG"])), subset_val)
            ],
            "MSK": [
                val["MSK"][u]
                for u in random.sample(range(0, len(val["IMG"])), subset_val)
        ...
        dict_test = {
            "IMG": [
                test["IMG"][u]
                for u in random.sample(range(0, len(test["IMG"])), subset_test)
            ],
            "MTD": [
                test["MTD"][u]
                for u in random.sample(range(0, len(test["IMG"])), subset_test)
