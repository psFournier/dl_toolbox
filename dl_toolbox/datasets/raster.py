...
class Raster(Dataset):
    ...
    def __init__(
        ...
        if shuffle:
            self.get_crop = RandomCropFromWindow(
                rasterio.windows.Window(0, 0, w, h), crop_size
            )
        else:
            self.get_crop = FixedCropFromWindow(
                rasterio.windows.Window(0, 0, w, h), crop_size, crop_step
            )
    ...
    def __getitem__(self, idx):
        ...
        image = self.read_image(self.img_path, crop, self.bands)

        label = None
        if self.label_path:
            label = self.read_label(self.label_path, crop, self.merge)
